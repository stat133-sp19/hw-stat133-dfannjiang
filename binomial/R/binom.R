...
#' @export
#' @examples
#' binomial probability distribution (with cumulative probabilities) using 20 trials and 45% chance of success on each trial
...
#' @export
#' @examples
#' # one success - probability of 3 successes in 5 trials with 50% chance of success on each trial
#' bin_probability(3, 5, 0.5)
#'
#' # multiple successes - probability of 3, 4 successes in 5 trials with 50% chance of success on each trial
...
bin_probability <- function(success, trials, prob) {
  ...
  return(bin_choose(trials, success) * (prob^success) * ((1 - prob)^(success - k)))
}
...
summary.binvar <- function(var, ...) {
  out <- list(
    trials = var$trials,
    prob = var$prob,
    mean = aux_mean(var$trials, var$prob),
    variance = aux_variance(var$trials, var$prob),
    mode = aux_mode(var$trials, var$prob),
    skewness = aux_skewness(var$trials, var$prob),
    kurtosis = aux_kurtosis(var$trials, var$prob)
  )
  return(out)
}
...
print.summary.binvar <- function(var, ...) {
  ...
  cat("Parameters\n")
  cat("- number of trials:", info$trials, "\n")
  cat("- prob of success :", info$prob, "\n\n")
  cat("Measures\n")
  cat("- mean    :", info$mean, "\n")
  cat("- variance:", info$variance, "\n")
  cat("- mode    :", info$mode, "\n")
  cat("- skewness:", info$skewness, "\n")
  cat("- kurtosis:", info$kurtosis, "\n")
}
